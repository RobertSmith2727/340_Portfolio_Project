...
app.put('/put-passenger-ajax', function(req,res,next){
    ...
    let queryUpdatePassenger = "UPDATE passengers SET passengerName = '" + passengerName +"', isNoFlightList= '" + isNoFlightList +"' WHERE idPassenger = '"+ idPassenger +"'";
...
app.put('/put-flight-ajax', function(req,res,next){
    ...
    let flightNumber = parseInt(data.flightNumber);
    let arrivalTime = String(data.arrivalTime);
    let departureTime = String(data.departureTime);
    let totalPassengers = parseInt(data.totalPassengers);
    

    let queryUpdateFlight = "UPDATE flights SET arrivalTime = '" + arrivalTime +"', departureTime= '" + departureTime +"', totalPassengers= '" + totalPassengers +"' WHERE flightNumber = '"+ flightNumber +"'";
    
  
          // Run the 1st query
          db.pool.query(queryUpdateFlight, function(error, rows, fields){
              if (error) {
